    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="数据名称">
          <el-input v-model="listQuery.filter1" placeholder="数据名称" />
        </el-form-item>
        <el-form-item label="数据类型">
          <el-input v-model="listQuery.filter2" placeholder="数据类型" />
        </el-form-item>
        <el-form-item label="数据状态">
          <el-select v-model="listQuery.filter3" placeholder="数据状态">
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="数据名称" prop="index" align="center" min-width="50">
          <template slot-scope="{ row }">
            <span>{{ row.index }}</span>
          </template>
        </el-table-column>
        <el-table-column label="数据类型" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="是否注册" prop="type1" align="center">
          <template slot-scope="{ row }">
            <el-tag :type="row.type1 == 0 ? '' : 'success'">{{ row.type1 == 0 ? '否' : '是' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="是否共享" prop="type2" align="center">
          <template slot-scope="{ row }">
            <el-tag :type="row.type2 == 0 ? 'danger' : 'success'">{{ row.type2 == 0 ? '否' : '是' }}</el-tag>
          </template>
        </el-table-column>
        <el-table-column label="当前状态" prop="type3" align="center">
          <template slot-scope="{ row }">
            <el-tag :type="row.type3 == 0 ? 'danger' : 'success'">{{ row.type3 == 0 ? '不正常' : '正常'
            }}</el-tag>
          </template>
        </el-table-column>
