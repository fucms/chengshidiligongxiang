    ...
    <div class="filter-container">
      ...
      <el-form :inline="true" :model="listQuery" class="demo-form-inline">
        <el-form-item label="图层名称">
          <el-input v-model="listQuery.filter1" placeholder="请输入图层名称" />
        </el-form-item>
        <el-form-item label="图层类型">
          <el-select v-model="listQuery.filter2" placeholder="请选择图层类型">
      ...
      <el-table
        ...
        style="width: 100%"
      >
        <el-table-column label="图层名称" prop="code" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.plan }}</span>
          </template>
        </el-table-column>
        <el-table-column label="图层类型" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.code }}</span>
          </template>
        </el-table-column>
        <el-table-column label="本周访问数" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.number }}</span>
          </template>
        </el-table-column>
        <el-table-column label="本月访问数" prop="type3" align="center">
          <template slot-scope="{ row }">
            <span>{{ row.price }}</span>
          </template>
        </el-table-column>
        <el-table-column label="本年度访问数" prop="type3" align="center">
          <template slot-scope="{ row }">
